...
abstract class Board extends Observable {
    ...
    boolean isLegal(Side player, int n) {
        return (isLegal(player) && player.playableSquare(get(n).getSide()));
    }
    ...
    final Side getWinner() {
        if (numOfSide(Side.RED) == size() * size()) {
            return Side.RED;
        } else if (numOfSide(Side.BLUE) == size() * size()) {
            return Side.BLUE;
        }
    ...
    public String toString() {
        ...
        for (int r = 1; r < size() + 1; r++) {
            ...
            for (int c = 1; c < size() + 1; c++) {
                tempSquare = get(r, c);
                dumpy += tempSquare.getSpots();
                tempSide = tempSquare.getSide().toString();
                if (tempSide.equals("white")) {
                    dumpy += "-";
                } else if (tempSide.equals("red")) {
                    dumpy += "r";
                } else {
                    dumpy += "b";
                }
