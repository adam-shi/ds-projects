...
class Game extends Observable {
    ...
    void makeMove(int r, int c) {
        try {
            if (!_board.exists(r, c)) {
                throw error("move " + r + " " + c + " out of bounds");
            }

            if (_board.isLegal(_board.whoseMove(), r, c)) {
                _board.addSpot(_board.whoseMove(), r, c);
            } else {
                throw error("invalid move: " + r + " " + c);
            }
        } catch (GameException g) {
            throw error(g.getMessage());
        }
    ...
    private void checkForWin() {
        ...
        if (_playing && (_board.numOfSide(_board.whoseMove().opposite())
    ...
    private void setAuto(Side color) {
        _playing = false;
        _players[color.ordinal() - 1] = new AI(this, color);
    }
    ...
    private void setSpots(int r, int c, int spots, String color) {
        ...
        if (!_board.exists(r, c)) {
            reportError("square " + r + " " + c + " does not exist.");
            return;
        }
        ...
        _board.set(r, c, spots, sideColor);
    }
    ...
    private void readExecuteCommand() {
        if (_inp.hasNextInt()) {
            try {
                ...
                int moveRow = _inp.nextInt();
                if (_inp.hasNextInt()) {
                    saveMove(moveRow, _inp.nextInt());
                    makeMove(_move[0], _move[1]);
                } else {
                    throw error("syntax error in move command");
                }
                _inp.nextLine();
            } catch (GameException moveG) {
                reportError(moveG.getMessage());
                _inp.nextLine();
            }
        } else {
    ...
    private void executeCommand(String cmnd) {
        ...
        case "auto":
            try {
                ...
                throw error("syntax error in '" + cmnd + "' command");
            }
            break;
        case "clear":
            clear();
        ...
        case "size":
            try {
                ...
                throw error("syntax error in 'size' command");
            }
            break;
        case "start":
            restartGame();
