...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        ...
        ArrayList<String> roadsTaken = new ArrayList<String>();
        ArrayList<Double> distances = new ArrayList<Double>();
        ArrayList<String> directions = new ArrayList<String>();
        Road currentRoad;

        for (int i = 0; i < segment.size() - 1; i++) {
            currentRoad = _map.getLabel(segment.get(i), segment.get(i + 1));
            roadsTaken.add(currentRoad.toString());
            distances.add(currentRoad.length());
            directions.add(currentRoad.direction().fullName());
        }
        ...
        for (int i = 0; i < roadsTaken.size(); i++) {
            tempName = roadsTaken.get(i);
            tempDist = distances.get(i);
            tempDir = directions.get(i);

            while (i + 1 < roadsTaken.size()
                   && roadsTaken.get(i + 1).equals(tempName)
                   && directions.get(i + 1).equals(tempDir)) {
                i += 1;
                tempDist += distances.get(i);
            }

            if (i != roadsTaken.size() - 1) {
                System.out.print("    " + seq + ". Take " + tempName + " "
                                 + tempDir + " for ");
                System.out.printf("%.1f ", tempDist);
                ...
                System.out.print("miles" + " to "
                                 + _map.getLabel(segment.get(j)).toString());
                System.out.println(".");
            }
            seq += 1;
        }
