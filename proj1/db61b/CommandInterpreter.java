...
class CommandInterpreter {
    ...
    void loadStatement() {
        ...
        _database.put(fileName, table);
        _input.next(";");
        System.out.println("Loaded " + fileName + ".db");
    }
    ...
    Table selectClause() {
        ...
        Table fromTable2 = null;

        if (_input.nextIf(",")) {
            twoTables = true;
            fromTable2 = tableName();
        }
        ...
        if (twoTables) {
            newTable = fromTable.findInnerJoin(fromTable2);
        } else {
            newTable = fromTable;
        }

        if (_input.nextIf("where")) {
            conditions = conditionClause(newTable);
        }
        newTable = newTable.select(columnNames, conditions);
        return newTable;
    ...
    /** Parse and return relational operator from the token stream. */
    String relational() {
    ...
    Condition condition(Table... tables) {
        ...
        Column column1 = new Column(col1, tables);

        String relation = relational();
        Condition c;
        String col2;

        if (_input.nextIs(Tokenizer.LITERAL)) {
            col2 = literal();
            c = new Condition(column1, relation, col2);
        } else {
            col2 = columnName();
            Column column2 = new Column(col2, tables);
            c = new Condition(column1, relation, column2);
        }
